...
subtitle: "Heavily edited sections from BTV1 _1. Introduction_"
...
#| fig-cap: The solar spectrum, $280-900\,\mathrm{nm}$, and different commonly used _named_ wavebands. The solar spectrum at ground level for 1.5 air masses (ASTM G173 standard) and, from top to bottom, human-vision colours according to ISO 21348, UV and IR bands according to ISO 21348, UV and IR bands according to CIE, colours as frequently defined in plant photobiology (following @Sellaro2010 and @Smith1981), and PAR [@McCree1972b]. 
...
Photosynthetically active radiation (PAR), is also an effective radiation quantity, proposed in the early 1970's @McCree1973. Before its widespread acceptance and the widespread availability of PAR or "quantum" light sensors. PAR is expressed in physical units as a flux of photons within 400 to 700 nm.

In the case of UV radiation, different biologically effective weightings are used, mostly to describe damage or inhibition. These quantities are normally expressed using energy flux units. However, these are _weighted_ values that do not describe an energy flux but instead a value assumed to be proportional to a response such as growth inhibition.
...
$$1 = R^\mathrm{tot} + A_\mathrm{fr} + T^\mathrm{tot}$$
where $R^\mathrm{tot}$, $A_\mathrm{fr}$, and $T^\mathrm{tot}$ are expressed as fractions [@tbl-light-matter-quantites]. Alternatively, they can be expressed as percentages.
...
|   Symbol         |       Unit       | Description                        |
|:----------------:|:----------------:|:-----------------------------------|
| $R^\mathrm{tot}$ | fraction         | Total reflectace:                  |
| $R^\mathrm{spcl}$ | fraction        | Specular reflectace:               |
| $R^\mathrm{diff}$ | fraction        | Diffuse reflectace:               |
| $n$              | numeric        | Refractive index:                    |
| $T^\mathrm{tot}$ | fraction         | Total transmittance:               |
| $T^\mathrm{int}$ | fraction        | Internal transmittance:             |
| $A_\mathrm{fr}$   | fraction         | Absorptance:               |
| $A$              | a.u.            | Absorbance:               |
| $xxx$            |                  | Molar attenuation:
...
The book _The Optics of Life_ [@Johnsen2012] gives a detailed account of the different aspects of optics as the relate to organisms. Although animales are emphasized more than plants in the examples, overall it is highly relevant plant biologists.
