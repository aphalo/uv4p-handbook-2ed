...
If we include twilight, the day-length estimates become longer.

```{r}
geocode_He <- data.frame(lon = 24.93838, lat = 60.16986)
hours <- seq(from = ymd_hm("2010-03-21 00:00", tz = "EET"),
             by = "1 min",
             length.out = 24 * 60)
...
ggplot(angles_He,
  ...
  geom_hline(data=twilight,
             aes(yintercept = angle, linetype=factor(location))) +
  ...
  labs(y = "Sun elevation at Helsinki (degrees)",
       x = "Time UTC", linetype = "Dawn/dusk") +
  theme_minimal()
```
...
```{r, echo=FALSE}
#| label: fig-AM-h
#| fig-cap: Plot of the empirical relationship between sun elevation ($h$) or zenith anglr ($\thets$) and "equivalent" atmosphere thickness traversed by solar radiation between outer space and the Earth's surface (at sea level).
...
The radiance ($L$) of radiation from the sun is similar, but not identical, to the emission spectrum of a *black body* at a temperature of 5800 K (@fig-blackbody-emission). 
...
ggplot(data=data.frame(x=c(50,2500)), aes(x)) +
...
The "colour temperature" of a black body is given by Wien's displacement law (@fig-wein-wl), which gives the peak wavelength of the radiation emitted by a black body as a function of its absolute temperature 
